...
This is a Claude Code **plugin** called **jira-auto-issue**. It provides automatic Jira issue tracking, work documentation, and time logging via hooks and slash commands. Every task done in Claude Code is tracked, documented, and logged to Jira with minimal manual intervention.

## Architecture

The plugin has four layers:

1. **Plugin manifest** (`.claude-plugin/marketplace.json` + `plugins/jira-auto-issue/.claude-plugin/plugin.json`) — registers the plugin with Claude Code.

2. **Slash commands** (`plugins/jira-auto-issue/commands/*.md`) — markdown files with YAML frontmatter:
...
3. **Hook handlers** (`plugins/jira-auto-issue/hooks-handlers/`) — shell scripts triggered by Claude Code lifecycle events:
...
4. **REST API client** (`plugins/jira-auto-issue/hooks-handlers/jira-rest.sh`) — curl-based Jira Cloud API functions as fallback when MCP tools are unavailable.
...
- **`.claude/jira-tracker.json`** — project config (committed): projectKey, cloudId, branchPattern, etc.
- **`.claude/jira-tracker.local.json`** — credentials (gitignored): email, apiToken, baseUrl
...
- Test hook scripts manually: `bash plugins/jira-auto-issue/hooks-handlers/session-start-check.sh` from a git repo with `.claude/jira-tracker.json` configured.
- Test REST client: `source plugins/jira-auto-issue/hooks-handlers/jira-rest.sh && jira_load_creds . && jira_test_connection`
